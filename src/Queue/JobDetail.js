...
const imgLink = "https://drive.ebrains.eu/media/avatars/default.png";
...
function JobDetail(props) {
  ...
  function handleSubmitComment() {
    ...
    const options = {
      method: "POST",
      url: `${jobQueueServer}${jobUrl}/comments/`,
      headers: {
    ...
    axios
      ...
      .then(function (response) {
        setCommentField("");
        const now = new Date();
        commentData["timestamp"] = now.toISOString();
        commentData["job_id"] = job.id;
        commentData["user_id"] = props.auth.tokenParsed["preferred_username"];
  ...
  function handleAddTag() {
    let requestUrl = `${jobQueueServer}${job.resource_uri}/tags/`;
    const options = {
      method: "post",
      url: requestUrl,
      headers: {
  ...
  function handleEditcomment() {
    ...
    const options = {
      method: "put",
      url: `${jobQueueServer}${jobUrl}/comment/${editedCommentId}`,
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + props.auth.token,
      },
  ...
  useEffect(() => {
    ...
    const jobUrl = `${jobQueueServer}/jobs/${id}?with_comments=true`;
  ...
  useEffect(() => {
    const tagsUrl = `${jobQueueServer}/tags/?collab=${job.collab}`;
  ...
  const getLog = async (jobId) => {
    const logUrl = `${jobQueueServer}/jobs/${jobId}/log`;
    const config = { headers: { Authorization: "Bearer " + props.auth.token } };
      ...
      <div style={{ marginBottom: "2%", marginTop: "1%" }}>
        <div style={{ marginLeft: "1%", marginRight: "2%" }}>
          <div style={{ display: "flex" }}>
            <Paper
              elevation={3}
              style={{ paddingLeft: "1%", paddingBottom: "0.1%", width: "30%", marginBottom: "1%" }}
            >
              <Box
                component="span"
                display="block"
              ...
              <p>
                <Box hover="true" component="span" display="block" fontSize="13px">
                  {" "}
                  Submitted on <strong>{timeFormat(job.timestamp_submission)}</strong> by{" "}
                  <strong>{job.user_id}</strong> to <strong>{job.hardware_platform}</strong>
                </Box>

                {job.status === "finished" || job.status === "error" ? (
                  <Box component="span" display="block" fontSize="13px">
                    {" "}
                    Completed on <strong> {timeFormat(job.timestamp_completion)}</strong>
                  </Box>
                ) : (
                  <Box component="span" display="block" fontSize="13px">
                    {" "}
            ...
            <div
              style={{ float: "left", paddingBottom: "2%", paddingLeft: "2%", paddingTop: "0.5%" }}
            >
              ...
              <div
                style={{ paddingBottom: "5%", paddingLeft: "2%", paddingTop: "0.5%" }}
                key="drive"
              ...
              >
                <Box sx={style}>
                  <Tooltip title="Copy output files to the Drive">
                    <Button
                      disabled={job.output_data && job.output_data.files.length > 0 ? false : true}
                      onClick={handleCopy2Drive}
                      style={{
                        color: "white",
                  ...
                  <ExportToDrive
                    auth={props.auth}
                    files={job.output_data ? job.output_data.files : []}
                    jobId={job.id}
                    collab={job.collab}
                    copy2drive={copy2drive}
                    openAlertCopy={openAlertCopy}
                    setOpenAlertCopy={setOpenAlertCopy}
                    openAlertDone={openAlertDone}
                    setOpenAlertDone={setOpenAlertDone}
                    openAlertSize={openAlertSize}
                    setOpenAlertSize={setOpenAlertSize}
                  />
                  ...
                  <Collapse in={openAlertSize}>
                    ...
                    >
                      Some files exceed the size limit of 1 GB. Please use the Bucket.
                    </Alert>
                  </Collapse>
                </Box>
              </div>
              ...
              >
                <Box sx={style}>
                  ...
                  <ExportToBucket
                    ...
                    openAlertCopy={openAlertCopy2}
                    setOpenAlertCopy={setOpenAlertCopy2}
                    openAlertDone={openAlertDone2}
                    setOpenAlertDone={setOpenAlertDone2}
                  />
              ...
              >
                ...
                <DialogContent>
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <TextField {...params} label="Attributed tags" variant="outlined" />
                    )}
                  />
                </DialogContent>
                <DialogActions>
          ...
          <Accordion defaultExpanded={true}>
            <AccordionSummary
              ...
              className={classes.expansion_panel_summary}
            >
              <Typography className={classes.heading}>
                <DescriptionIcon /> Output files{" "}
              </Typography>
            </AccordionSummary>
            <AccordionDetails className={classes.expansion_panel_details}>
              {job.output_data && job.output_data.files.length > 0
                ? job.output_data.files.map((out_file, index) => (
                    <div
                      style={{ marginTop: "0.5%", display: "flex", marginLeft: "1%" }}
                      key={out_file.url}
                    >
          ...
          <Accordion defaultExpanded={true}>
            ...
            >
              <Typography className={classes.heading}>
                <StorageIcon /> Hardware Config
              </Typography>
            </AccordionSummary>
            <AccordionDetails className={classes.expansion_panel_details}>
              <List component="nav" aria-label="mailbox folders">
                <ListItem Box>
                  <Box component="div" display="inline" width="300px">
                    Platform:
                  </Box>
          ...
          <Accordion defaultExpanded={true}>
            ...
            <AccordionDetails className={classes.expansion_panel_details}>
              <Box component="div" display="inline" width="300px" style={{ paddingLeft: "15px" }}>
                Machine's IP :
              </Box>
              <Box component="div" display="inline">
                {job.provenance ? String(job.provenance.spinnaker_machine) : "No details"}
              </Box>
            </AccordionDetails>
          </Accordion>
        ...
        {comments.map((comment) => (
          <div key={comment.id}>
            <Paper
              elevation={3}
              className={classes.comments_panel}
              style={{
                marginLeft: "1%",
                paddingLeft: "1%",
                paddingBottom: "0.1%",
            ...
            >
              <Grid container wrap="nowrap" spacing={2}>
                ...
                <Grid
                  ...
                  zeroMinWidth
                  onClick={() => {
                    if (props.auth.tokenParsed["preferred_username"] === comment.user) {
                      setOpenCommentEdit(true);
                      setEditedCommentId(comment.id);
                      setEditedCommentField(comment.content);
                    }
                  }}
                >
                  ...
                  <Typography variant="body2" color="gray">
                    {`Posted on ${timeFormat(comment.timestamp)} by ${comment.user_id}`}
                  </Typography>
                </Grid>
              </Grid>
            </Paper>
            ...
            >
              ...
              <DialogContent>
                <TextField
                  ...
                  onChange={(event) => {
                    setEditedCommentField(event.target.value);
                  }}
