  ...
  return (
    <div className={classes.root}>
      ...
      <div>
        <p>
          <span
            className={job.status === "finished" ? "badge badge-success" : "badge badge-danger"}
          >
            {job.status}
          </span>
        </p>
        <p>
          <small>
            ...
            by <strong>{job.user_id}</strong> to <strong>{job.hardware_platform}</strong>
          </small>
          <br></br>
          <small>
            Completed on{" "}
            <strong>
              {" "}
              {String(job.timestamp_completion).slice(0, 4) +
                "/" +
                String(job.timestamp_completion).slice(5, 7) +
                "/" +
                String(job.timestamp_completion).slice(8, 10) +
                " " +
                String(job.timestamp_completion).slice(11, 19)}
            </strong>
          </small>
        </p>
        ...
        <ExpansionPanel defaultExpanded={true}>
          ...
          <ExpansionPanelDetails className={classes.expansion_panel_details}>
            {job.output_data && job.output_data.length > 0
              ? job.output_data.map((out_file) => (
                  <div>
                    <p>
                      {" "}
                      <a href={String(out_file.url)}> {String(out_file.url)} </a> {"\n"}{" "}
                    </p>
                  </div>
                ))
              : "No files available"}
        ...
        <ExpansionPanel defaultExpanded={true}>
          ...
          <ExpansionPanelDetails className={classes.expansion_panel_details}>
            {job.provenance ? "Machine's IP : " + job.provenance.spinnaker_machine : "No details"}
          </ExpansionPanelDetails>
        </ExpansionPanel>
